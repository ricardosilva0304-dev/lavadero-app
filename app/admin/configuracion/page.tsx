...
export default function ConfiguracionPage() {
  ...
  const actualizarTarifa = async (tipo: 'carro' | 'moto', precios: any) => {
    const datosActualizados = {
      precio_hora: Number(precios.hora || 0),
      precio_noche: Number(precios.noche || 0),
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 p-4 md:p-8 lg:p-10 pb-32">
      ...
      <header className="max-w-7xl mx-auto mb-8 lg:mb-12">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 md:gap-6">
          ...
          <div>
            ...
            <p className="text-slate-400 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.4em] mt-1">Configuración del sistema</p>
          </div>
        </motion.div>
      </header>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10">
        ...
        {/* NAVEGACIÓN: Scroll horizontal en móvil, sidebar en PC */}
        <nav className="lg:col-span-3 flex lg:flex-col gap-3 overflow-x-auto pb-4 lg:pb-0 no-scrollbar sticky top-0 z-30 bg-[#F8FAFC]/80 backdrop-blur-md lg:bg-transparent -mx-4 px-4 lg:mx-0 lg:px-0">
              ...
              {activeTab === 'servicios' && (
                <div className="space-y-10">
                  <div className="bg-slate-50 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-inner">
                    <h2 className="text-lg md:text-xl font-black text-slate-800 mb-6 md:mb-8 italic uppercase tracking-tighter">Nuevo Servicio Comercial</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                      ...
                      <div className="space-y-2">
                        ...
                        <select className="w-full bg-white border-2 border-slate-100 p-4 md:p-5 rounded-2xl md:rounded-3xl outline-none font-bold text-slate-600 appearance-none"
...
// --- COMPONENTES AUXILIARES CON RESPONSIVIDAD ---
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full border-2 p-4 rounded-xl md:rounded-[1.5rem] outline-none transition-all font-bold text-sm ${dark
            ...
            : 'bg-white border-slate-100 text-slate-900 focus:border-gorilla-orange'
            } ${icon ? 'pl-11' : ''}`}
        />
      </div>
    </div>
...
function TarifaCard({ tipo, icono, valores, onSave }: any) {
  const [vals, setVals] = useState({ dia: 0, mes: 0, hora: 0, noche: 0 })
  ...
  return (
    <div className="bg-white border-2 border-slate-100 p-6 rounded-[2rem] md:rounded-[3rem] shadow-lg">
...
function MiniInput({ label, value, onChange }: any) {
  return (
    <div className="space-y-1">
      ...
      <div className="relative">
        <DollarSign className="absolute left-2 top-1/2 -translate-y-1/2 text-gorilla-orange" size={10} />
        <input
          type="number"
          value={value}
          onChange={(e: any) => onChange(Number(e.target.value))}
          className="w-full bg-slate-50 border-none p-2.5 pl-5 rounded-lg text-xs font-black outline-none focus:ring-1 focus:ring-orange-500/30 text-slate-900"
        />
      </div>
    </div>
  )
...
function TabButton({ active, onClick, icon, label, color }: any) {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 px-6 py-4 lg:py-6 rounded-2xl lg:rounded-[2rem] font-black uppercase text-[10px] tracking-widest transition-all border-2 whitespace-nowrap ${active
