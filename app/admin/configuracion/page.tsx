...
import { 
  ...
  UserPlus, Layers, Save, X, Edit3, Clock, Check
} from 'lucide-react'
...
export default function ConfiguracionPage() {
  ...
  const fetchData = async () => {
    ...
    if (trf) {
      const tMap = { carro: 0, moto: 0 }
      trf.forEach(t => { if(t.tipo_vehiculo === 'carro' || t.tipo_vehiculo === 'moto') tMap[t.tipo_vehiculo] = t.precio_hora })
      setTarifas(tMap)
    }
    setLoading(false)
  }
                ...
                <div className="space-y-12">
                  <div className="bg-slate-50 p-10 rounded-[3rem] border border-slate-100 shadow-inner">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <FormInput label="Nombre" placeholder="Ej: Full Detail" value={formServicio.nombre} onChange={e => setFormServicio({...formServicio, nombre: e.target.value})} />
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Vehículo</label>
                        <select className="w-full bg-white border-2 border-slate-100 p-5 rounded-3xl focus:border-gorilla-orange outline-none font-bold text-slate-600 cursor-pointer"
                    ...
                    <div className="grid grid-cols-2 gap-8 mt-8">
                        {(formServicio.aplica_a === 'carro' || formServicio.aplica_a === 'ambos') && (
                            <FormInput label="Tarifa Carro" type="number" icon={<DollarSign size={18}/>} onChange={e => setFormServicio({...formServicio, precio_carro: Number(e.target.value)})} />
                        )}
                        {(formServicio.aplica_a === 'moto' || formServicio.aplica_a === 'ambos') && (
                            <FormInput label="Tarifa Moto" type="number" icon={<DollarSign size={18}/>} onChange={e => setFormServicio({...formServicio, precio_moto: Number(e.target.value)})} />
                        )}
                    </div>
                ...
                <div className="space-y-12">
                  <div className={`p-10 rounded-[3.5rem] shadow-2xl transition-colors duration-500 relative overflow-hidden ${editandoUser ? 'bg-gorilla-orange/10 border-gorilla-orange/20' : 'bg-slate-900'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                      <FormInput dark label="Nombre Completo" value={formUsuario.nombre} onChange={e => setFormUsuario({...formUsuario, nombre: e.target.value})} />
                      <FormInput dark label="Cédula" value={formUsuario.cedula} onChange={e => setFormUsuario({...formUsuario, cedula: e.target.value})} />
                      <FormInput dark label="Celular" value={formUsuario.telefono} onChange={e => setFormUsuario({...formUsuario, telefono: e.target.value})} />
                      <div className="space-y-2">
                        <label className="text-[9px] font-black text-slate-500 uppercase ml-2">Rol</label>
                        <select className="w-full bg-white/10 border border-white/10 p-5 rounded-[1.5rem] text-white font-bold outline-none cursor-pointer"
                ...
                <div className="space-y-12">
                  <div className="flex flex-col md:flex-row gap-8">
                     <TarifaCard 
                        ...
                        icono={<Car size={40}/>} 
                        valor={tarifas.carro} 
                        onSave={(v) => actualizarTarifa('carro', v)} 
                     />
                     <TarifaCard 
                        tipo="moto" 
                        icono={<Bike size={40}/>} 
                        valor={tarifas.moto} 
                        onSave={(v) => actualizarTarifa('moto', v)} 
                     />
                  </div>
                  <div className="p-8 bg-blue-50 rounded-[2.5rem] border border-blue-100 flex items-start gap-4">
                     <Info className="text-blue-500 shrink-0 mt-1" size={20}/>
                     <p className="text-xs font-bold text-blue-600 leading-relaxed uppercase italic">Los cambios en las tarifas se aplicarán de inmediato a todos los vehículos que ingresen al parqueadero a partir de este momento.</p>
                  </div>
                </div>
              )}
  ...
  return (
    <div className="space-y-2 flex-1">
      ...
      <div className="relative">
        ...
        <input 
            {...props} 
            className={`w-full border-2 p-5 rounded-[1.5rem] outline-none transition-all font-bold ${
                dark 
    ...
    return (
        <div className="flex-1 bg-white border-2 border-slate-100 p-10 rounded-[3.5rem] shadow-xl hover:shadow-2xl transition-all group">
            ...
            <button 
                ...
                className={`w-full p-5 rounded-2xl font-black uppercase text-xs tracking-widest flex items-center justify-center gap-2 transition-all ${saved ? 'bg-green-500 text-white' : 'bg-slate-900 text-white hover:bg-black shadow-lg'}`}
            >
...
function TabButton({ active, onClick, icon, label, color }: any) {
  ...
  return (
    <button 
      onClick={onClick}
      className={`flex items-center gap-4 px-8 py-7 rounded-[2.5rem] font-black uppercase text-[11px] tracking-[0.2em] transition-all duration-500 whitespace-nowrap border-2 ${
        active 
